...
Geodesic GeodesicIntegrator::integrate(const double x0[4], const double p0[4],
    ...
    if (!verify_initial_conditions(x, geodesic.E_initial, geodesic.L_initial, init_error)) {
        geodesic.termination = TerminationReason::INSTABILITY;
    ...
    while (lambda < lambda_max) {
        ...
        auto rhs_wrapper = [&](double lam, const double* s, double* ds) {
            // Reconstruct full 4D state (t and phi are ignorable)
            double x_full[4] = {0.0, s[0], s[1], 0.0};
        ...
        // Horizon proximity scaling
        if (r < 12.0) {
             scale = std::min(scale, dist_horizon / 10.0);
        }
        
        // Photon sphere proximity scaling
        if (dist_photon < 1.0) {
        ...
        // Constraint stabilization (Hamiltonian H=0)
...
bool GeodesicIntegrator::verify_initial_conditions(const double x[4], double E, double L, std::string& error_msg) const {
    ...
    // Null geodesic potential: V_eff = (1 - 2M/r) * (L^2/r^2)
    
    double one_minus_2M_r = 1.0 - 2.0 * M / r;
    double L2_r2 = (L * L) / (r * r);
    double V_eff = one_minus_2M_r * L2_r2;
    
    // Check if region is classically forbidden (E^2 < V_eff)
...
void GeodesicIntegrator::stabilize_constraints(double x[4], double p[4], double E_target) const {
    // Project momenta to satisfy H_kin = -H_pot
    
    double H_pot = -0.5 * (1.0 / (1.0 - 2.0/x[Physics::R])) * (E_target * E_target);
    ...
    if (H_kin > 1e-10) {
        ...
        // Scale dynamic momenta (p_r, p_theta)
        p[Physics::R] *= scale;
...
void GeodesicIntegrator::compute_diagnostics(double lambda, const double x[4], const double p[4],
    ...
    // Carter Constant Q (p_theta^2 for Schwarzschild)
...
TerminationReason GeodesicIntegrator::check_termination(const double x[4], const double p[4],
    ...
    // Dynamic escape threshold
    const double escape_threshold = std::max(1.5 * initial_r, 10.0 * R_PHOTON_SPHERE);
    
    // Check for escape (beyond threshold moving outward)
    ...
    // Capture heuristic (inside photon sphere moving inward)
    if (r < 3.0 && p[R] < 0.0) {
        return TerminationReason::HORIZON_CROSSED;
    }
    ...
    if (lambda >= lambda_max) {
        return (r > 25.0) ? TerminationReason::ESCAPED : TerminationReason::MAX_LAMBDA;
    }
